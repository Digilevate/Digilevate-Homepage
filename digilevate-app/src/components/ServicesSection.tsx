...
const ServiceCard: React.FC<{ service: Service }> = ({ service }) => {
    const { t } = useTranslation();
    const serviceKey = service.id === 1 ? 'starter_site' : service.id === 2 ? 'business_pro' : 'e_commerce';
    ...
    return (
        <div className={`glass-effect rounded-2xl p-8 transition-all duration-300 relative flex flex-col ${service.popular ? 'ring-2 ring-[#2d9bf0] scale-105 hover:ring-4' : 'hover:-translate-y-2 hover:shadow-2xl hover:shadow-[#2d9bf0]/20'}`}>
            ...
            <div className="text-center mb-6">
                <h3 className="text-2xl font-bold text-white mb-2">{t(`services.${serviceKey}.name`)}</h3>
                <p className="text-gray-400 mb-4 h-10">{t(`services.${serviceKey}.tagline`)}</p>
                <div className="text-3xl font-bold gradient-text mb-2">
                    {t('services_section.from') + ` CHF ${service.price_from.toLocaleString('de-CH')}.-`}
                </div>
                <p className="text-sm text-gray-400">{t(`services.${serviceKey}.ideal_for`)}</p>
            </div>
            ...
            <ul className="space-y-3 mb-8 flex-grow">
                {service.features.map((_, idx) => (
                    <li key={idx} className="flex items-start space-x-3">
                        ...
                        <span className="text-gray-300 text-sm">{t(`services.${serviceKey}.features.${idx}`)}</span>
                    </li>
                ))}
            </ul>

            <a href="#contact" className={`w-full text-center group py-3 px-4 rounded-lg transition-all duration-300 inline-block mt-auto ${service.popular
